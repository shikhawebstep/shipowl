...
import { getProductRequestById, removeProductRequestImageByIndex } from '@/app/models/admin/product/myProductRequest';
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const productRequestResult = await getProductRequestById(Number(adminId), String(adminRole), productRequestId);
    ...
    const result = await removeProductRequestImageByIndex(Number(adminId), String(adminRole), productRequestId, imageIndex);
