...
import { cancelOrderShipping } from '@/utils/order/cancelOrderShipping';
...
export async function GET(req: NextRequest) {
    try {
        ...
        const shippingResponse = await cancelOrderShipping(orderId);
        ...
        logMessage('info', 'Order shipping cancelled successfully', { orderId, shippingResult });

        return NextResponse.json({
            status: true,
            message: 'Order shipping cancelled successfully',
            data: shippingResult
        }, { status: 200 });
