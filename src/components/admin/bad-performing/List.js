        ...
        if (typeof window !== "undefined" && data.length > 0 && !loading) {
            ...
            Promise.all([
                ...
                .then(([jQuery]) => {
                    ...
                    if ($.fn.DataTable.isDataTable("#badPincodes")) {
                        $("#badPincodes").DataTable().destroy();
                        $("#badPincodes").empty();
                    }
    ...
    return (
        <div className="bg-white rounded-3xl md:w-8/12 p-5">
            <div className="flex flex-wrap justify-between items-center mb-4">
                <h2 className="md:text-2xl font-bold text-[#2B3674]">Bad Pincodes List</h2>
                        ...
                        {isPopupOpen && (
                            <div className="absolute md:left-0 mt-2 w-40 right-0 bg-white rounded-md shadow-lg z-10">
                                <ul className="py-2 text-sm text-[#2B3674]">
                                    <li className="px-4 md:hidden block py-2 hover:bg-gray-100 cursor-pointer">  {canViewTrashed && <button
                    ...
                    <div className="flex justify-between items-center mb-2">
                        ...
                        <button
                            onClick={() => {
                                ...
                                if (window.$.fn.DataTable.isDataTable('#badPincodes')) {
                                    window.$('#badPincodes').DataTable().column(activeFilter.columnIndex).search('').draw();
                                }
                            }}
            ...
            {data.length > 0 ? (
                <div className="overflow-x-auto relative main-outer-wrapper w-full">
                    <table className="md:w-full w-auto display main-tables" id="badPincodes">
                        <thead>
                                ...
                                <tr key={item.id} className="border-b border-[#E9EDF7] text-[#2B3674] font-semibold">
                                    ...
                                    <td className="p-2 px-5 text-right">
                                        ...
                                        <div className="flex justify-center gap-2">{isTrashed ? (
                                            ...
                                            <>
                                                {canEdit && <Pencil onClick={() => router.push(`/admin/bad-pincodes/update?id=${item.id}`)} className="cursor-pointer text-3xl" />}
                                                {canSoftDelete && (
