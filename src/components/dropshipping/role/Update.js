...
export default function Update() {
    ...
    const fetchRole = useCallback(async () => {
        const dropshippingData = JSON.parse(localStorage.getItem("shippingData"));

        if (dropshippingData?.project?.active_panel !== "dropshipper") {
            localStorage.removeItem("shippingData");
            router.push("/dropshipping/auth/login");
            return;
        }

        const dropshippingtoken = dropshippingData?.security?.token;
        if (!dropshippingtoken) {
            router.push("/dropshipping/auth/login");
            return;
        }
        ...
        try {
            ...
            const response = await fetch(
                `${process.env.NEXT_PUBLIC_API_BASE_URL}api/dropshipper/role/${id}`,
                {
                    method: "GET",
                    headers: {
                        "Content-Type": "application/json",
                        Authorization: `Bearer ${dropshippingtoken}`,
                    },
                }
            );
    ...
    const handleSubmit = async (e) => {
        ...
        const token = dropshippingData?.security?.token;
        if (!token) {
            router.push("/dropshipping/auth/login");
            return;
        }
        ...
        try {
            ...
            const url = `${process.env.NEXT_PUBLIC_API_BASE_URL}api/dropshipper/role/${id}`;
            ...
            Swal.fire({
                ...
                if (res.isConfirmed) {
                    ...
                    router.push("/dropshipping/permission/role");
                }
            });
        } catch (error) {
