...
import { useSupplierAction } from '@/components/commonfunctions/SupplierMainContext'
...
export default function List() {
    ...
    const canDelete = hasPermission("Product", "Permanent Delete");
    const canAdd = hasPermission("Product", "Create");
    ...
    const { fetchAll, fetchTrashed, softDelete, restore, destroy } = useSupplierAction("supplier/role", "roles");
                        ...
                        <div className="flex gap-3  items-center">
                                ...
                                <button
                                    onClick={async () => {
                                        await handleBulkDelete({
                                            selected,
                                            apiEndpoint: `${process.env.NEXT_PUBLIC_API_BASE_URL}api/supplier/permission/role/bulk`,
                                            setSelected,
                                            setLoading,
                                        });
                                        await fetchAll(setRoleData, setLoading);
                            ...
                            <div className="md:flex w-full justify-end gap-2">
                                ...
                                {canAdd && (
                                    ...
                                    >
                                         <Link href="/supplier/permission/role/create"> Add Role</Link>
                                    </button>
                                )}
                            </div>
                            <button
                                ...
                                {isPopupOpen && (
                                    <div className="absolute right-0 mt-2 w-40 bg-white rounded-md shadow-lg z-10">
                                        <ul className="py-2 text-sm text-[#2B3674] text-left">
                                            ...
                                            <li className="px-4 block md:hidden py-2 hover:bg-gray-100 cursor-pointer"><Link href="/supplier/permission/role/create"> Add Role</Link></li>
                                                        ...
                                                        {isTrashed ? (
                                                            ...
                                                            <>
                                                                {canEdit && (
                                                                    <Pencil
                                                                        onClick={() =>
                                                                            router.push(`/supplier/permission/role/update?id=${item.id}`)
                                                                        }
