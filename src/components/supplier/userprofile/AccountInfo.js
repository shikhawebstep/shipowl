...
const AccountInfo = () => {
  ...
  const handleImageDelete = async (index, type, id) => {
    ...
    try {
      ...
      const url = `${process.env.NEXT_PUBLIC_API_BASE_URL}api/supplier/${formData.id}/bank-account/${id}/image/${index}?type=${type}`;
  ...
  const handleSubmit = async (e) => {
    if (validate()) {
      ...
      try {
        ...
        const url = `${process.env.NEXT_PUBLIC_API_BASE_URL}api/supplier/profile/update`; // Ensure the URL is correct
