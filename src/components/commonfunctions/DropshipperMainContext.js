// contexts/DropshipperActionContext.js
import { useRouter } from "next/navigation";
import React, { createContext, useContext, useCallback } from "react";
import Swal from "sweetalert2";


export const DropshipperActionContext = createContext();

export const DropshipperActionProvider = ({ children }) => {
  ...
  return (
    <DropshipperActionContext.Provider value={{ getAuthToken }}>
      {children}
    </DropshipperActionContext.Provider>
  );
};

export const useDropshipperAction = (baseEndpoint, resultKey = "data") => {
  const context = useContext(DropshipperActionContext);
  if (!context) {
    throw new Error("useDropshipperAction must be used within an DropshipperActionProvider");
  }
