// contexts/SupplierActionContext.js
import { useRouter } from "next/navigation";
import React, { createContext, useContext, useCallback } from "react";
import Swal from "sweetalert2";


export const SupplierActionContext = createContext();

export const SupplierActionProvider = ({ children }) => {
  ...
  return (
    <SupplierActionContext.Provider value={{ getAuthToken }}>
      {children}
    </SupplierActionContext.Provider>
  );
};

export const useSupplierAction = (baseEndpoint, resultKey = "data") => {
  const context = useContext(SupplierActionContext);
  if (!context) {
    throw new Error("useSupplierAction must be used within an SupplierActionProvider");
  }
