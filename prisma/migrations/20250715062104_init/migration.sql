...
CREATE TABLE `warehouse` (
    ...
    INDEX `warehouse_supplierId_idx`(`supplierId`),
...
-- CreateTable
CREATE TABLE `dropshipperBanner` (
    `id` INTEGER NOT NULL AUTO_INCREMENT,
    ...
    INDEX `dropshipperBanner_createdBy_idx`(`createdBy`),
    INDEX `dropshipperBanner_updatedBy_idx`(`updatedBy`),
    INDEX `dropshipperBanner_deletedAt_idx`(`deletedAt`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- AddForeignKey
ALTER TABLE `warehouse` ADD CONSTRAINT `warehouse_supplierId_fkey` FOREIGN KEY (`supplierId`) REFERENCES `admin`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;
